...
class DeepFashionDataset(Pix2pixDataset):
    ...
    def get_paths(self, opt):
        ...
        for i in range(len(lines)):
            name = lines[i].strip()
            image_paths.append(name.replace('\\', "/"))
            label_path = name.replace('img', 'pose').replace('.jpg', '_{}.txt').replace('\\', "/")
            label_paths.append(os.path.join(label_path))
        return label_paths, image_paths

    def get_ref_video_like(self, opt):
        pair_path = os.path.join(opt.dataroot, 'deepfashion_self_pair.txt')
        ...
        ref_path = 'deepfashion_ref_test.txt' if opt.phase == 'test' else 'deepfashion_ref.txt'
        ref_path = os.path.join(opt.dataroot, ref_path)
    ...
    def get_ref_vgg(self, opt):
        ...
        if opt.phase == 'test':
            extra = '_test'
        with open(os.path.join(opt.dataroot, 'deepfashion_ref{}.txt'.format(extra))) as fd:
            lines = fd.readlines()
        ref_dict = {}
        for i in range(len(lines)):
    ...
    def get_label_tensor(self, path):
        ...
        stickwidth = 4
        limbSeq = [[2, 3], [2, 6], [3, 4], [4, 5], [6, 7], [7, 8], [2, 9], [9, 10], [10, 11], [2, 12], [12, 13], [13, 14], [2, 1], [1, 15], [15, 17],
