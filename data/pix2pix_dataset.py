...
class Pix2pixDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        label_path = os.path.join(self.opt.maskroot, label_path)
        ...
        if random_p < self.real_reference_probability or self.opt.phase == 'test':
            # key = image_path.split('deepfashion/')[-1] if self.opt.dataset_mode == 'deepfashion' \
            #     else os.path.basename(image_path)
            key = image_path.split('metfaces/')[-1]
            ...
            if random_p < self.hard_reference_probability:
                ...
                #easy reference
                path_ref = val[1]
            if self.opt.dataset_mode == 'deepfashion' or self.opt.dataset_mode == 'metfaces':
